...
from pydantic import BaseModel # Enable us to use models in FastAPI, in a easy way
from typing import List # Enable us to use some types, like the List type
...
async def reservar_estoque(op: OperacoesEstoque):
    for operacao in op.operacoes:
        produto = estoque.get(operacao.sku)
        if not produto: # if product == None
